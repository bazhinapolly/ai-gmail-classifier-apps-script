...
def strip():
    data = [
        ...
        [p("Google Apps Script", "h3"), p("Strict Structured Outputs", "h3"), p("Minimized and redacted input", "h3"), p("18 tests + multi-version CI", "h3")],
    ]
...
def build_case_study():
    story = [
        ...
        p("18 deterministic tests cover minimized input, PII redaction, strict output validation, confidence fallback, refusal and incomplete responses, retry classification, atomic label updates, and spreadsheet formula safety. GitHub Actions repeats syntax, manifest, hygiene, and test checks on Node.js 20, 22, and 24."),
    ...
    document(OUT / "AI-Gmail-Classifier-Case-Study.pdf", "AI Gmail Classifier - Case Study").build(story, onFirstPage=frame, onLaterPages=frame)
...
def build_technical():
    story = [
        ...
        p("Verification", "h2"),
        p("18 tests plus repository checks run on Node.js 20, 22, and 24. A dedicated Apps Script smoke-test function exercises one controlled provider request before trigger activation."),
        p("Run locally", "h2"),
    ...
    document(OUT / "AI-Gmail-Classifier-Technical-Summary.pdf", "AI Gmail Classifier - Technical Summary").build(story, onFirstPage=frame, onLaterPages=frame)
